...
//this function add an item into the cart
  const addItem = ( item ) => {
    ...
    if( added ) {
      ...
      setCartItems( cartItems.map( (val) => val.id === item.id ? { ...added, quantity: added.quantity + 1 }: val ) );
    } else {
...
//this function removes an item or decrease item count by 1
  ...
  return (
    <div className='App'>
      <Cart addItem={ addItem } removeItem={ removeItem } buyBye={ buyBye } cartItems={ cartItems } closeCart={ closeCart }/>
